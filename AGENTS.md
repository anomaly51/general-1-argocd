...
- Keep macOS Wi-Fi DNS pointed at the LAN resolver `192.168.1.2` (the GitOps-managed `internal-dns` load balancer); after DNS changes, run `dscacheutil -flushcache` before validation. Do not replace the LAN resolver with public DNS servers because it forwards public queries upstream.
